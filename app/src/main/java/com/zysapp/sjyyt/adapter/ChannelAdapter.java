...
public class ChannelAdapter extends BaseRecycleAdapter<Channel> {
    ...
    protected void bindData(BaseViewHolder holder, final int position) {
        ...
        if (datas.get(position).isCheck())
            ((ImageView) holder.getView(R.id.iv_check)).setImageResource(R.mipmap.img_check_p);
        else
            ((ImageView) holder.getView(R.id.iv_check)).setImageResource(R.mipmap.img_check_n);
